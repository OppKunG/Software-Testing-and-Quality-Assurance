...
public class CalculationController {
    ...
    public ResponseEntity<?> multiply(@PathVariable("number1") String number1, @PathVariable("number2") String number2) {
        {
            try {
                int result = calculationService.multiply(number1, number2);
                return ResponseEntity.ok(result);
            } catch (Exception ex) {
                ex.printStackTrace();
                return ResponseEntity.badRequest().body(ex.getMessage());
    ...
    public ResponseEntity<?> divide(@RequestParam String number1, @RequestParam String number2) {
        {
            int divide = calculationService.divide(number1, number2);
            if (divide != -1) {
